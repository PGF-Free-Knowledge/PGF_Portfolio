...
with tabs[2]:
    ...
    with c1:
        ...
        fig.update_layout(height=410, margin=dict(l=10,r=90,t=10,b=35), xaxis_title="Impacto medio absoluto SHAP", yaxis_title="", template="plotly_white")
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": True})
    ...
    with c2:
        ...
        fig2.update_layout(height=410, margin=dict(l=10,r=90,t=10,b=35), xaxis_title="Contribución SHAP", yaxis_title="", template="plotly_white")
        st.plotly_chart(fig2, use_container_width=True, config={"displayModeBar": True})
        m1,m2=st.columns(2)
...
with tabs[3]:
    ...
    st.markdown("#### Selección del escenario")
    preset = st.selectbox(
        "Modo de exploración",
        ["Manual", "Reducir HVAC 10 %", "Reducir IT Load 10 %", "Aumentar IT Load 10 %"],
        index=0,
    )
    c1,c2=st.columns(2)
    with c1:
        default_it = baseline_it * (0.90 if preset == "Reducir IT Load 10 %" else 1.10 if preset == "Aumentar IT Load 10 %" else 1.0)
        it=st.slider("IT Load [kW]",float(test["it_load_kW"].min()),float(test["it_load_kW"].max()),float(np.clip(default_it,float(test["it_load_kW"].min()),float(test["it_load_kW"].max()))), disabled=(preset != "Manual"))
    with c2:
        default_hv = baseline_hv * 0.90 if preset == "Reducir HVAC 10 %" else baseline_hv
        hv=st.slider("HVAC [kW]",float(test["hvac_kW"].min()),float(test["hvac_kW"].max()),float(np.clip(default_hv,float(test["hvac_kW"].min()),float(test["hvac_kW"].max()))), disabled=(preset != "Manual"))
    scenario=base_row.copy(); scenario["it_load_kW"]=it; scenario["hvac_kW"]=hv
    if preset != "Manual":
        st.info(f"Escenario predefinido: **{preset}**. Úsalo como punto de partida y luego vuelve a Manual para explorar libremente.")
    ...
    fig.update_layout(height=360,margin=dict(l=20,r=20,t=45,b=20),yaxis_title="Consumo predicho [kW]",template="plotly_white",showlegend=False)
    fig.add_annotation(x=0.5,y=max(baseline,scenario_y),text=f"Δ = {delta:+.2f} kW",showarrow=True,arrowhead=2,yshift=18)
    st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":True})
