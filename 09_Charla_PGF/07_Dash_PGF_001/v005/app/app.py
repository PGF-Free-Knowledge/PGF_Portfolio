...
with tabs[2]:
    ...
    with c1:
        ...
        fig = go.Figure(go.Bar(orientation="h", x=top.values, y=[x.replace("_"," ") for x in top.index],
                               hovertemplate="%{y}<br>Impacto medio: %{x:.3f}<extra></extra>"))
    ...
    with c2:
        ...
        fig2 = go.Figure(go.Bar(orientation="h",
                                x=local_top.values[::-1],
                                y=[x.replace("_"," ") for x in local_top.index[::-1]],
                                hovertemplate="%{y}<br>Contribución: %{x:.3f}<extra></extra>"))
        fig2.add_vline(x=0, line_width=1)
        fig2.update_layout(height=360, margin=dict(l=10,r=10,t=10,b=30),
                           xaxis_title="Contribución SHAP", yaxis_title="", template="plotly_white")
        st.plotly_chart(fig2, use_container_width=True, config={"displayModeBar": True})

        st.metric("Consumo real", f"{row['total_consumption_kW']:.2f} kW")
        st.metric("Consumo predicho", f"{pred[idx]:.2f} kW")

    st.markdown(
        '<div class="callout"><b>Lectura:</b> SHAP permite pasar de “el modelo predijo” '
        'a “estas variables contribuyeron a esa predicción”. No implica causalidad física.</div>',
        unsafe_allow_html=True
    )
...
with tabs[3]:
    ...
    st.write(
        "Modifica variables de operación y observa la respuesta del modelo. "
        "Es una demostración metodológica; no representa ahorro validado de Oil Malal."
    )
    ...
    for col,label,value,note in [
        (k1,"BASE",f"{baseline:.2f} kW","predicción de referencia"),
        (k2,"ESCENARIO",f"{scenario_y:.2f} kW","predicción modificada"),
    ...
    fig = go.Figure(go.Bar(
        x=["Base", "Escenario"], y=[baseline, scenario_y],
        text=[f"{baseline:.2f} kW", f"{scenario_y:.2f} kW"],
        textposition="outside",
        hovertemplate="%{x}<br>%{y:.2f} kW<extra></extra>"
    ))
    fig.update_layout(height=330, margin=dict(l=20,r=20,t=35,b=20),
                      yaxis_title="Consumo predicho [kW]", template="plotly_white")
    st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": True})
    ...
    if delta < 0:
        ...
        msg = "El escenario no cambia la predicción respecto de la base."
    st.markdown(f'<div class="callout"><b>Comparación:</b> {msg}</div>', unsafe_allow_html=True)

    st.markdown(
        '<div class="callout"><b>Lectura:</b> la demostración permite formular “¿qué pasa si...?” '
        'antes de intervenir. El resultado es una predicción del modelo, no una medición de ahorro.</div>',
        unsafe_allow_html=True
    )
