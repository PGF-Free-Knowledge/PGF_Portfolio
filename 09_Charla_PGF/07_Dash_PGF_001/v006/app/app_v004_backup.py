...
with tabs[0]:
    ...
    ax.plot(fdf["timestamp"], fdf["total_consumption_kW"], label="Consumo total")
    ax.set_ylabel("kW")
...
with tabs[2]:
    ...
    with c2:
        ...
        fig2, ax2 = plt.subplots(figsize=(5.8,4.7))
        ax2.barh([x.replace("_"," ") for x in local_top.index[::-1]], local_top.values[::-1])
        ax2.axvline(0, linewidth=1)
        ax2.set_xlabel("Contribución SHAP")
...
with tabs[3]:
    ...
    ax.bar(["Base","Escenario"], [baseline, scenario_y])
    ax.set_ylabel("Consumo predicho [kW]")
